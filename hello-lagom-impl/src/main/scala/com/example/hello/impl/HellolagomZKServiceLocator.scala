...
object HellolagomZKServiceLocator {
  case class Config(serverHostname:String,
    ...
    def zkUri = HellolagomZKServiceLocator.zkUri(serverHostname,serverPort)
  }
...
class HellolagomZKServiceLocator @Inject() (serverHostname:String,
                                            serverPort:Int,
                                            scheme:String,
                                            routingPolicy:String,
  ...
  import com.lightbend.lagom.discovery.zookeeper.ZooKeeperServiceLocator._

  @Inject()
  def this(config: HellolagomZKServiceLocator.Config)(implicit ec: ExecutionContext) =
  ...
  override def locate(name: String): Future[Option[URI]] = locateAsScala(name)
  ...
  override def locate(name: String, serviceCall: Call[_, _]): Future[Option[URI]] = locateAsScala(name)
